...
Nous allons donc centralisé la structure de notre page dans un fichier `layout.edge` présent dans `ressources/views/components/`.
...
Avec {{ }} : Edge envoie `&lt;strong&gt;Salut&lt;/strong&gt;`. Le navigateur affiche le texte : <strong>Salut</strong>.

Avec {{{ }}} : Edge envoie `<strong>Salut</strong>`. Le navigateur affiche : Salut.
